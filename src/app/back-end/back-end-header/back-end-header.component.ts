...
export class BackEndHeaderComponent implements OnInit {
  ...
  fixProfileUrl(profilUrl: string | null | undefined): string {
    ...
    if (!profilUrl) {
      return "assets/profile.png"; // Gambar default jika profilUrl tidak ada
    }
  
    // Cek apakah profilUrl sudah lengkap (termasuk domain)
    if (profilUrl.startsWith("http://") || profilUrl.startsWith("https://")) {
      return profilUrl; // Jika sudah lengkap, kembalikan URL seperti adanya
    }
  
    // Jika hanya path, tambahkan BASE_BACKEND_URL
